...
int main(int argc,char**argv)
{
	...
	fclose(fp);
	CDem ReferenceSurface;
	if(ReferenceSurface.Open(strReferenceSurfaceFile)==false)return false;
	...
	/* 确定未知数的初始值                                                   */
	/************************************************************************/
	double lfFai = 0.0,lfOmig = 0.0,  lfKafa = 0.0, lfScale = 1.0, lfTx = 0.0, lfTy = 0.0, lfTz = 0.0;
	printf("\n七参数的初始值：\n");
	printf("lfTx=%lf\tlfTy=%lf\tlfTz=%lf\tlfScale=%lf\tRy=%lf\tRx=%lf\tRz=%lf\n", lfTx, lfTy, lfTz, lfScale, lfFai, lfOmig, lfKafa);
	/************************************************************************/
	/* 进入迭代过程                                                         */
	/************************************************************************/
	...
	while (true)
	{
		...
		{
			//常数项矩阵
			matrixL(i, 0) = -(ptCenteredReferenceSurfacePt[i].z - ptCenteredFeaturePt[i].z);
		}
		for (int i = 0; i < iFeaturePtNums; i++)
		{
			//法矢矩阵
		...
		{
			//常系数矩阵
			matrixB(i * 3, 0) = 1.0; matrixB(i * 3, 1) = 0.0; matrixB(i * 3, 2) = 0.0; matrixB(i * 3, 3) = ptCenteredFeaturePt[i].x; matrixB(i * 3, 4) = -ptCenteredFeaturePt[i].z; matrixB(i * 3, 5) = 0.0; matrixB(i * 3, 6) = -ptCenteredFeaturePt[i].y;
			matrixB(i * 3 + 1, 0) = 0.0; matrixB(i * 3 + 1, 1) = 1.0; matrixB(i * 3 + 1, 2) = 0.0; matrixB(i * 3 + 1, 3) = ptCenteredFeaturePt[i].y; matrixB(i * 3 + 1, 4) = 0.0; matrixB(i * 3 + 1, 5) = -ptCenteredFeaturePt[i].z; matrixB(i * 3 + 1, 6) = ptCenteredFeaturePt[i].x;
			matrixB(i * 3 + 2, 0) = 0.0; matrixB(i * 3 + 2, 1) = 0.0; matrixB(i * 3 + 2, 2) = 1.0; matrixB(i * 3 + 2, 3) = ptCenteredFeaturePt[i].z; matrixB(i * 3 + 2, 4) = ptCenteredFeaturePt[i].x; matrixB(i * 3 + 2, 5) = ptCenteredFeaturePt[i].y; matrixB(i * 3 + 2, 6) = 0.0;
		}
		//cout << "A\n" << matrixA << endl;
		//cout << "B\n" << matrixB << endl;
		//cout << "L\n" << matrixL << endl;
		...
		/* 检查计算是否收敛                                                     */
		/************************************************************************/
		if (iIterator>30 || (fabs(matrixX(0, 0)) < 0.01 && fabs(matrixX(1, 0)) < 0.01 && fabs(matrixX(2, 0)) < 0.01&&fabs(matrixX(3, 0))<0.05 
			&& fabs(matrixX(4, 0))<0.5 && fabs(matrixX(5, 0))<0.5 && fabs(matrixX(6, 0))<0.5)) break;
		/************************************************************************/
		/* 累加                                                                 */
		/************************************************************************/
		lfTx += matrixX(0, 0);
