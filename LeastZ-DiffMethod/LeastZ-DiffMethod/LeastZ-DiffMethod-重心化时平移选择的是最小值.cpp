...
int main(int argc,char**argv)
{
	...
	for (int i = 0; i < iFeaturePtNums; i++)
	{
		ptIniCenteredFeaPt[i].x -= lfFeaPtXmin;
		ptIniCenteredFeaPt[i].y -= lfFeaPtYmin;
		ptIniCenteredFeaPt[i].z -= lfFeaPtZmin;
	}
	//然后进行缩放
	for (int i = 0; i < iFeaturePtNums; i++)
	{
		lfFeaturePtCenterX += ptIniCenteredFeaPt[i].x;
		lfFeaturePtCenterY += ptIniCenteredFeaPt[i].y;
		lfFeaturePtCenterZ += ptIniCenteredFeaPt[i].z;
	}
	lfFeaturePtScaleX = lfFeaturePtCenterX / (lfFeaturePtCenterZ*10);
	lfFeaturePtScaleY = lfFeaturePtCenterY / (lfFeaturePtCenterZ*10);
	for (int i = 0; i < iFeaturePtNums; i++)
	{
	...
	CGdalDem *pRefDemFile=new CGdalDem; GDALDEMHDR *pReffDemHead=new GDALDEMHDR;
	if (false == pRefDemFile->LoadFile(strReferenceSurfaceFile, pReffDemHead)){ printf("%s: %s\n",ERROR_FILE_OPEN,strReferenceSurfaceFile); return false; }
	...
	while (true)
	{
		...
		for (int i = 0; i < iFeaturePtNums; i++)
		{
			...
			RefPtTmp.x += lfFeaPtXmin;
			RefPtTmp.y += lfFeaPtYmin;
		...
		{
			ptCenteredReferenceSurfacePt[i].x -= lfRefPtXmin;
			ptCenteredReferenceSurfacePt[i].y -= lfRefPtYmin;
			ptCenteredReferenceSurfacePt[i].z -= lfRefPtZmin;
		}
		//然后进行缩放
		for (int i = 0; i < ptReferenceSurfacePt.size(); i++)
		{
		...
		lfRefPtScaleX = lfReferenceSurfaceCenterX / (lfReferenceSurfaceCenterZ*10);
		lfRefPtScaleY = lfReferenceSurfaceCenterY / (lfReferenceSurfaceCenterZ*10);
		for (int i = 0; i < ptReferenceSurfacePt.size(); i++)
		{
		...
		for (int i = 0; i < ptReferenceSurfacePt.size(); i++)
		{
			...
			//matrixB(i * 3, 0) = 0.0; matrixB(i * 3, 1) = ptSourceFeaPt[i].z; matrixB(i * 3, 2) = -ptSourceFeaPt[i].y; matrixB(i * 3, 3) = ptSourceFeaPt[i].x; matrixB(i * 3, 4) = 1.0; matrixB(i * 3, 5) = 0.0; matrixB(i * 3, 6) = 0.0;
			//matrixB(i * 3 + 1, 0) = -ptSourceFeaPt[i].z; matrixB(i * 3 + 1, 1) = 0.0; matrixB(i * 3 + 1, 2) = ptSourceFeaPt[i].x; matrixB(i * 3 + 1, 3) = ptSourceFeaPt[i].y; matrixB(i * 3 + 1, 4) = 0.0; matrixB(i * 3 + 1, 5) = 1.0; matrixB(i * 3 + 1, 6) = 0.0;
			//matrixB(i * 3 + 2, 0) = ptSourceFeaPt[i].y; matrixB(i * 3 + 2, 1) = -ptSourceFeaPt[i].x; matrixB(i * 3 + 2, 2) = 0.0; matrixB(i * 3 + 2, 3) = ptSourceFeaPt[i].z; matrixB(i * 3 + 2, 4) = 0.0; matrixB(i * 3 + 2, 5) = 0.0; matrixB(i * 3 + 2, 6) = 1.0;
		}*/
		for (int i = 0; i < ptReferenceSurfacePt.size(); i++)
		{
			//常数项矩阵
	...
	fclose(fp);
	delete pRefDemFile, pReffDemHead;
	return true;
}