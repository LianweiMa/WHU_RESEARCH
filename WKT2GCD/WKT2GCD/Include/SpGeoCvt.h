...
#define MakeLambert2Scale(Std_Par1,Std_Par2)  ( int(Std_Par1*10000000000.0)+Std_Par2 )
#define GetLambert1(Scale)  ( int(Scale)*1.0/10000000000.0 )
#define GetLambert2(Scale)  ( Scale- 10000000000.0*(int(Scale)*1.0/10000000000.0) )
...
public:
    ...
    static void Cvt_TPC2LBH_WGS84( double Xt,double Yt,double Zt,double Initial_B,double Initial_H,double L0,double B0,double H0,double *L,double *B,double *H )
    {
...
//      double N,WGS84_A,WGS84_B,WGS84_C,WGS84_D,WGS84_E;
		
		do{
			*L = temp_L;    
			*B = temp_B;    
			*H = temp_H;			
			double N = WGS84_a/sqrt(1-WGS84_e*WGS84_e*sin(*B)*sin(*B));
			
			double WGS84_A = (1+H0/N0)*cos(B0)-sin(B0)/N0*Yt+cos(B0)/N0*Zt;
			double WGS84_B = (1+H0/N0)*sin(B0)+cos(B0)/N0*Yt+sin(B0)/N0*Zt;
			double WGS84_C = sqrt(WGS84_A*WGS84_A + pow(Xt/N0,2));
			double WGS84_D = WGS84_e*WGS84_e*(N0*sin(B0)-N*sin(*B));
			double WGS84_E = sqrt(1+2*WGS84_D*sin(*B)/(N+*H)+pow(WGS84_D/(N+*H),2));
			
			temp_L = atan(Xt/(N0*WGS84_A))+L0;
			temp_B = atan(WGS84_B/WGS84_C-WGS84_D/(N0*WGS84_C));
			temp_H = Xt/(cos(*B)*sin(temp_L-L0))-N;
    ...
    static void Cvt_LBH2TPC_WGS84( double L,double B,double H,double L0,double B0,double H0,double *Xt,double *Yt,double *Zt )
    {
        double sinB  = sin(B);
        double cosB  = cos(B);
        double sinB0 = sin(B0);
...
public:
    CSpPar7Cvt()
	{ 
		m_l2wDx=m_l2wDy=m_l2wDz=0; m_l2wScale=0; 
    ...
    void   Set_Cvt_Par( double dx,double dy,double dz,double rx,double ry,double rz,double scale)
	{
    ...
    void    Get_Cvt_Par( double *dx,double *dy,double *dz,double *rx,double *ry,double *rz,double *scale)
	{
		*dx=m_l2wDx; 
		*dy=m_l2wDy; 
		*dz=m_l2wDz;
...
public:
    ...
    void    Set_Cvt_Par( double lon0,double lat0,double dlon,double dlat,int nlon,int nlat,float *pDz )
	{
		...
		if ( m_pDz ) 
			delete m_pDz;
		m_pDz = new float[nlon*nlat +8]; 
		memcpy( m_pDz,pDz,sizeof(float)*nlon*nlat );
	};
    ...
    BOOL    Save2File(LPCSTR lpstrPathName);
    LPCSTR  GetFilePN(){ return m_strPathName; };
    float   Get_Alt2Geo(double lon,double lat)
	{
		if ( m_dLat==0||m_dLon==0 ) return 0;
