...
public:
    ...
    void    Set_Cvt_Par( double lon0,double lat0,double dlon,double dlat,int nlon,int nlat,float *pDz ){
                m_Lon0 = lon0; m_Lat0 = lat0; m_dLon = dlon; m_dLat = dlat; 
                m_nLon = nlon; m_nLat = nlat;    if ( m_pDz ) delete m_pDz;
                m_pDz = new float[nlon*nlat +8]; memcpy( m_pDz,pDz,sizeof(float)*nlon*nlat );
            };
