...
float GetMzxZvalue(double x, double y, CGdalDem*pDemList_i, int &nDemNum, double &lfXmin, double &lfYmin, double &lfXmax, double &lfYmax)//distance as weight
{
...
int main(int argc, char**argv)
{
	...
	for (int i = 0; i < iRows; i++)
	{
		...
		{
			*(pMosaicDemData + i*iCols + j) = GetMzxZvalue(x, y, pDemList, iDemNum, lfXmin, lfYmin, lfXmax, lfYmax);
			x += lfXgsd;
		}
		y -= lfYgsd;
