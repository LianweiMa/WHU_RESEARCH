...
{
	printf("\n-----Strat Convert Background Color-----\n");
	//read image file
	...
	BYTE  *pData = new BYTE[nWidth*nHeight*nBands];
	...
	gdalInputDataSet->RasterIO(GF_Read, 0, 0, nWidth, nHeight, pData, nWidth, nHeight, GDT_Byte, nBands, 0, 0, 0, 0);
	for (int i = 0; i < nHeight; i++)
	{
		for (int j = 0; j < nWidth; j++)
		{
			///*if (*(pData + i*nWidth + j) == 0)*/*(pData + i*nWidth + j) = 255;
	...
	sprintf(strrchr(strOutputFile, '.'), "%s", "_result.bmp");
	GDALDriver *poDriver;
	...
	gdalOutputDataSet->RasterIO(GF_Write, 0, 0, nWidth, nHeight, pData, nWidth, nHeight, GDT_Byte, nBands, 0, 0, 0, 0);
