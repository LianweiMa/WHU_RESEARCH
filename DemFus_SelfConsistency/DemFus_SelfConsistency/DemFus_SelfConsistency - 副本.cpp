...
int main(int argc, char**argv)
{
		...
		for (int j = 0; j < iCols; j+=BLOCK_WIDTH_MLW)
		{
			...
			for (int i_Block = 0; i_Block < Block_Height; i_Block++)
			{
				for (int j_Block = 0; j_Block < Block_Width; j_Block++)
				{
					...
					if (fZ1 == NOVALUE_MLW || fZ2 == NOVALUE_MLW)*(pfDiff_Z + i_Block*Block_Width + j_Block) = NOVALUE_MLW;
					else
					{
	...
	for (int i = 0; i < iRows; i += BLOCK_HEIGHT_MLW)
	{
		for (int j = 0; j < iCols; j += BLOCK_WIDTH_MLW)
		{
			...
			demDiff.ReadBlock(pfOriBlock_Z, j, i, Block_Width, Block_Height);
			float fZ_Tmp = NOVALUE_MLW;
			for (int i_Block = 0; i_Block < Block_Height; i_Block++)
			{
				for (int j_Block = 0; j_Block < Block_Width; j_Block++)
				{
					fZ_Tmp = *(pfOriBlock_Z + i_Block*Block_Width + j_Block);
					if (fZ_Tmp <= NOVALUE_MLW){
						*(pfMarkedBlock_Z + i_Block*Block_Width + j_Block) = NOVALUE_MLW;
					}
					else if (fZ_Tmp >2 * fDiffStdDev)
					{
						*(pfMarkedBlock_Z + i_Block*Block_Width + j_Block) = fZ_Tmp;
						iMarkedNums++;
					}
				}
			}
	...
	int iNumTmp = 0;
	iMarkedNums = 0;
	//write fused dem for reliable point
	for (int i = 0; i < iRows; i += BLOCK_HEIGHT_MLW)
	{
		for (int j = 0; j < iCols; j += BLOCK_WIDTH_MLW)
		{
			...
			for (int i_Block = 0; i_Block < Block_Height; i_Block++)
			{
				for (int j_Block = 0; j_Block < Block_Width; j_Block++)
				{
					...
					if (fZ_Tmp == fNoMarked){//no mark
						...
						else { *(pfFusedBlock_Z + i_Block*Block_Width + j_Block) = MARK_VALUE_MLW; iMarkedNums++; }
					}
					else { *(pfFusedBlock_Z + i_Block*Block_Width + j_Block) = MARK_VALUE_MLW; iMarkedNums++; }//mark//no processing for this step
				}
			}
	...
	printf("Update MarkedNums:%d\n", iMarkedNums);
	//fill novalue for fused dem
	printf("--Filling novalue--\n");
	...
	while (iMarkedNums>0)
	{
			...
			for (int j = 0; j < iCols; j += BLOCK_WIDTH_MLW)
			{
				...
				for (int i_Block = 0; i_Block < Block_Height; i_Block++)
				{
					for (int j_Block = 0; j_Block < Block_Width; j_Block++)
					{
						lfFusedDemGeoX = lfStartFusedDemGeoX + j_Block*demFused.GetDemHeader().intervalX;
						lfFusedDemGeoY = lfStartFusedDemGeoY - i_Block*demFused.GetDemHeader().intervalY;
						//float fMarked = *(pfMarkedBlock_Z + i_Block*Block_Width + j_Block);

						if (fMarked != fNoMarked){//mark//processing for this step
							fZ_Tmp = demFused.GetDemZValue(lfFusedDemGeoX, lfFusedDemGeoY);//linear interpolated//near interpolated
							if (fZ_Tmp != NOVALUE_MLW){
								*(pfFilledBlock_Z + i_Block*Block_Width + j_Block) = fZ_Tmp;
								//*(pfMarkedBlock_Z + i_Block*Block_Width + j_Block) = fNoMarked;
								iMarkedNums--;
							}
						}
					}
				...
				demMarked.WriteBlock(pfMarkedBlock_Z, j, i, Block_Width, Block_Height);
				demMarked.WriteBlock(pfFilledBlock_Z, j, i, Block_Width, Block_Height);
				delete[]pfFilledBlock_Z;
				delete[]pfMarkedBlock_Z;
			}
		}
		/*int iMarkedStatisNums = 0;
		for (int iS = 0; iS < iRows; iS += BLOCK_HEIGHT_MLW)
		{
			for (int jS = 0; jS < iCols; jS += BLOCK_WIDTH_MLW)
			{
				int Block_Height = 0, Block_Width = 0;
				if (iS + BLOCK_HEIGHT_MLW <= iRows)
				{
					if (jS + BLOCK_WIDTH_MLW <= iCols)
					{
						Block_Height = BLOCK_HEIGHT_MLW;
						Block_Width = BLOCK_WIDTH_MLW;
					}
				...
				float *pfOriBlock_Z = new float[Block_Height*Block_Width];
				memset(pfOriBlock_Z, 0, sizeof(float)*Block_Height*Block_Width);
				demMarked.ReadBlock(pfOriBlock_Z, jS, iS, Block_Width, Block_Height);
				float fZ_Tmp = NOVALUE_MLW;
				for (int i_Block = 0; i_Block < Block_Height; i_Block++)
				{
					for (int j_Block = 0; j_Block < Block_Width; j_Block++)
					{
						fZ_Tmp = demMarked.GetDemZValue(jS + j_Block, iS + i_Block);
						if (fZ_Tmp !=NOVALUE_MLW&&fZ_Tmp!=0.0f ){  iMarkedStatisNums++; }
					}
				}
				delete[]pfOriBlock_Z;
			}
		...
		printf("MarkedNums:%d--\n", iMarkedNums);
		//printf("MarkedNums:%d MarkedStatis:%d--\n", iMarkedNums, iMarkedStatisNums);
	}
