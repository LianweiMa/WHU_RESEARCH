...
int main(int argc, char**argv)
{
	...
	for (int i = 0; i < iRows - 2; i++)
	{
		for (int j = 0; j < iCols - 2; j++)
		{
			...
			grid[3].fZ = *(pDemData + (i + 1)*iCols + j); grid[3].iID = (i + 1)*iCols + j;
			grid[4].fZ = *(pDemData + (i + 1)*iCols + j + 1); grid[4].iID = (i + 1)*iCols + j + 1;
			grid[5].fZ = *(pDemData + (i + 1)*iCols + j + 2); grid[5].iID = (i + 1)*iCols + j + 2;
			grid[6].fZ = *(pDemData + (i + 2)*iCols + j); grid[6].iID = (i + 2)*iCols + j;
			grid[7].fZ = *(pDemData + (i + 2)*iCols + j + 1); grid[7].iID = (i + 2)*iCols + j + 1;
			grid[8].fZ = *(pDemData + (i + 2)*iCols + j + 2); grid[8].iID = (i + 2)*iCols + j + 2;
			if (grid[0].fZ == NODATA || grid[1].fZ == NODATA || grid[2].fZ == NODATA ||
				grid[3].fZ == NODATA || grid[4].fZ == NODATA || grid[5].fZ == NODATA || 
				grid[6].fZ == NODATA || grid[7].fZ == NODATA || grid[8].fZ == NODATA  )continue;
			{ ZID t; for (int m = 0; m<9; m++){ for (int n = m + 1; n<4; n++){ if (grid[m].fZ<grid[n].fZ){ t = grid[n]; grid[n] = grid[m]; grid[m] = t; } } } }
			*(pMarkedData + grid[0].iID) = 255;
			*(pMarkedData + grid[8].iID) = 0;
		}
	}
