...
#define  SRTM_PRECISE 9.519416//unit:meter
#define  ASTER_PRECISE 10.098296//unit:meter
float GetFusedZvalue(double x, double y, CGdalDem*pDemList_i, int &nDemNum)//Accuracy as weight
{
	...
	pfZ_Temp = pfZ_Old;
	if (numZ == 2)return float(((1.0f / SRTM_PRECISE)*pfZ_Temp[0] + (1.0f / ASTER_PRECISE)*pfZ_Temp[1]) / ((1.0f / SRTM_PRECISE) + (1.0f / ASTER_PRECISE)));
	else if (numZ == 1)
	{
...
int main(int argc, char**argv)
{
	...
	for (int i = 0; i < iRows; i++)
	{
		...
		{
			*(pFusedDemData + i*iCols + j) = GetFusedZvalue(x, y, pDemList, iDemNum);
			x += lfXgsd;
		}
		y -= lfYgsd;
