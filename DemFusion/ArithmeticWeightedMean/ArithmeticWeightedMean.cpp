...
float GetFusedZvalue(double &lfX_i, double &lfY_i, CGdalDem*pDemList_i, int &iDemNum_i, CGdalDem*pSrtm_i)//arithmetic weighted mean
{
...
int main(int argc, char**argv)
{
	...
	for (int i = 0; i < iRows; i++)
	{
		...
		{
			*(pFusedDemData + i*iCols + j) = GetFusedZvalue(x, y, pDemList, iDemNum, pSrtm);
			x += lfXgsd;
		}
		y -= lfYgsd;
